sdeTiTm7 <- function(data, yTi,Ph){
  ...
  model$addSystem(dTi ~  (1/Ri2*(yTi2-Ti)+1/Ci*(1/Ria*(Ta-Ti)+ + 1/Rim*(Tm-Ti) + Ph + (a7*bs7+a1*bs1+a2*bs2+a3*bs3+a4*bs4+a5*bs5+a6*bs6)*Gv))*dt + exp(p11)*dw1)
  model$addSystem(dTm ~  1/Cm*(1/Rim*(Ti-Tm))*dt + exp(p22)*dw2)
  # Set the names of the inputs
  model$addInput(Ta,Gv,Ph, bs1, bs2,bs3,bs4,bs5, bs6, bs7, yTi2)
  ...
  model$setParameter(Cm = c(init = 250, lb = 1E-5, ub = 1E7))
  ...
  model$setParameter(a1 = c(init = 85, lb = 1E-2, ub = 200))
  ...
  model$setParameter(Ri2 = c(init = 10, lb = 0.001, ub = 30))
